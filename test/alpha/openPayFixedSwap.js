...
contract("Greenwood Alpha - openPayFixedSwap", async accounts => {
    it("should open a pay fixed swap for msg.sender", async () => {
        ...
        const postMetrics = await metricInstance.getMetrics.call();

        assert.notEqual(preContractState.avgFixedRateReceiving.toString(), postContractState.avgFixedRateReceiving.toString(), 'avgFixedRateReceiving before should not equal avgFixedRateReceiving after');
        assert.notEqual(preContractState.fixedToReceive.toString(), postContractState.fixedToReceive.toString(), 'fixedToReceive before should not equal fixedToReceive after');
        ...
        assert.equal('pFix', swapType.toString(), 'swapType should equal pFix');
    ...
    it("should not open a pay fixed swap for msg.sender because the contract is paused", async () => {
        const coreInstance = await core.deployed();
        ...
        await truffleAssert.fails(coreInstance.openSwap('100000000000000000000', 'pFix'), truffleAssert.ErrorType.REVERT);
    ...
    it("should not open a pay fixed swap for msg.sender because the swap notional is 0", async () => {
        const coreInstance = await core.deployed();
        await truffleAssert.fails(coreInstance.openSwap('0', 'pFix'), truffleAssert.ErrorType.REVERT);
    });
});
